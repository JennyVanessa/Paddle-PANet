...
class PAN_CTW(Dataset):
    ...
    def prepare_train_data(self, index):
        ...
        return img.astype(np.float32), gt_text.astype(np.float32), gt_kernels.astype(np.float32), training_mask.astype(np.float32), gt_instance.astype(np.float32), gt_bboxes.astype(np.float32)
