...
def _resnet(
    ...
    if pretrained:
        load_dygraph_pretrain(model, "pretrained/resnet18")
    return model
...
if __name__ == "__main__":
    ...
    img=paddle.to_tensor(imgs0)
    model = resnet18()

    # with open('torch_resnet18.pkl', 'rb') as f:
    #     param2 = pickle.load(f)
    # model.set_state_dict(param2)
    model.eval()

    output = model(paddle.to_tensor(img))
    print(output)
    # np.save('paddle_resnet18.npy', output.numpy())
